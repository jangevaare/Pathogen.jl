...
η: external infection pressure rate
...
α, β: powerlaw infection kernel parameters
η: external infection pressure rate
γ: removal rate (1/mean infectious period)
"""
function SIR_loglikelihood(α::Float64,
  ...
  # Create event timing array
  event_times = [aug.infectious aug.removed]

  # Find event order
  event_order = sortperm(event_times[:])

  # Create empty rate array
  rate_array = fill(0., (1 + length(obs.covariates) + 1, length(obs.covariates)))
  ...
  for i = 1:length(event_order)
    ...
    if id[2] == 1
      ...
      # Record infection rates at time of infection
      network_rates[:, id[1]] = rate_array[1:(size(rate_array, 2) + 1), id[1]]
      ...
      # Update removal rate
      rate_array[1 + size(rate_array, 2) + 1, id[1]] = γ
  ...
  debug && println("SIR loglikelihood: $ll")
  return ll, network_rates
end
