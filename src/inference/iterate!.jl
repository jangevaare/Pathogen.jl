...
function iterate!(mc::MarkovChain{S, M},
  ...
  for i = 1:n
    if use_adapted_Σrp
      update!(mc, mcmc, adapted_Σrp, σ, condition_on_network = condition_on_network, event_batches = event_batches)
    else
      update!(mc, mcmc, Σrp, σ, condition_on_network = condition_on_network, event_batches = event_batches)
    end
    next!(pmeter)
...
function iterate!(mc::MarkovChain{S, M},
  ...
  for i = 1:n
    if use_adapted_Σrp & use_adapted_Σsm
      update!(mc, mcmc, adapted_Σrp, adapted_Σsm, σ, condition_on_network = condition_on_network, event_batches = event_batches)
    elseif use_adapted_Σrp & !use_adapted_Σsm
      update!(mc, mcmc, adapted_Σrp, Σsm, σ, condition_on_network = condition_on_network, event_batches = event_batches)
    elseif !use_adapted_Σrp & use_adapted_Σsm
      update!(mc, mcmc, Σrp, adapted_Σsm, σ, condition_on_network = condition_on_network, event_batches = event_batches)    
    elseif !use_adapted_Σrp & !use_adapted_Σsm
      update!(mc, mcmc, Σrp, Σsm, σ, condition_on_network = condition_on_network, event_batches = event_batches)
    end
    next!(pmeter)
    @logmsg LogLevel(-5000) "MCMC progress" progress = i/n
    if adapt_cov > 0 && mod(i, adapt_cov) == 0
...
function iterate!(mcmc::MCMC{S, M},
  ...
  if length(mcmc.markov_chains) == 1
    ...
    @debug "Starting MCMC..."
    for mc in mcmc.markov_chains
      push!(mc_Futures, @spawn iterate!(mc, mcmc, n, Σrp, Σsm, σ, pchannel, condition_on_network = condition_on_network, event_batches = event_batches, adapt_cov = adapt_cov))
    end
    @debug "MCMC in progress..."
    while !all(isready.(mc_Futures))
      take!(pchannel) && next!(pmeter)
