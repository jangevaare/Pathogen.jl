function simulate!(sim::Simulation{T}; tmax::Float64=Inf, nmax::Int64=1000, pmax::Float64=Inf) where T <: EpidemicModel
  ...
  elseif pmax <= 0.0
    ...
    println("Simulating until a simulation time maximum of $tmax, an interation maximum of $nmax, or a processing time maximum of $pmax seconds is reached, whichever comes first.")
  end
  stoptime = time() + pmax
  while true
    next!(sim)
