...
function propose(i::Int64,
  ...
  elseif j == 2
    infection_lb = maximum([exposed[i];
                            observations.infected[i]-event_extents.infection])
...
function propose(i::Int64,
  ...
  if j == 1
    if length(pathto) > 1
      ...
      infection_lb = observations.infected[i]-event_extents.infection
