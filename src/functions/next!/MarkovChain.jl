function next!(mc::MarkovChain{T},
               ...
               batch_size::Int64=1) where T <: EpidemicModel
  # Initialize
  if batch_size < 0
  ...
  event_batches = cld(length(aug_order), batch_size)
  for i = 1:(event_batches + 1)
    if i <= event_batches
      for j = (batch_size*(i-1) + 1):minimum([(batch_size*i + 1); length(aug_order)])
        ...
        last_event = Event{T}(time, id, new_state)
        ...
        if condition_on_network
          if _new_transmission(last_event)
